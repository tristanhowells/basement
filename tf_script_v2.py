...
#             weights = self.model.load_weights(r'/storage/new_test/')
            ...
            self.model.self.model.load_weights(r'/storage/new_test/')
       
            #Target Model - Predict this model every step
            self.target_model = self.create_model()
            self.target_model.self.model.load_weights(r'/storage/new_test/')
        else:
...
#             print("reward type", type(reward), "episode_reward type", type(episode_reward), "ep_reward type", type(ep_rewards))
    ...
    if not episode % AGGREGATE_STATS_EVERY or episode == 1:
        ...
        if min_reward >= MIN_REWARD:
            agent.model.save(f'/artifacts/models{today}/{MODEL_NAME}__{max_reward:_>7.2f}max_{average_reward:_>7.2f}avg_{min_reward:_>7.2f}min__{int(time.time())}.h5')
            model_tag = f'/artifacts/models{today}/{MODEL_NAME}__{max_reward:_>7.2f}max_{average_reward:_>7.2f}avg_{min_reward:_>7.2f}min__{int(time.time())}.h5'
            print(model_tag)
        if episode%10 == 0:
            agent.model.save(f'/artifacts/models{today}/{MODEL_NAME}__{max_reward:_>7.2f}max_{average_reward:_>7.2f}avg_{min_reward:_>7.2f}min__{int(time.time())}.h5')
            model_tag = f'/artifacts/models{today}/{MODEL_NAME}__{max_reward:_>7.2f}max_{average_reward:_>7.2f}avg_{min_reward:_>7.2f}min__{int(time.time())}.h5'
