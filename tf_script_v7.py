#altered reward functiom

#current stable test model
...
###LOAD EXISTING MODEL
LOAD_MODEL = None #r'/storage/models/latest_May-29-2021/_0' 
...
class Trader:
    ...
    def action(self, choice): 
        ...
        if choice == 2:
            if self.volume <= 0: #if there is no volume to sell, pass 
                self.reward = TIME_STEP_PENALTY
                self.volume = self.volume
                self.current_value = self.current_value
                self.purchase_price = self.purchase_price
...
#             print("portfoilo_vs_market: ", portfolio_vs_market)
            market_change_list.append(market_change)
            portfolio_change_list.append(portfolio_change)
            portfolio_vs_market_list.append(portfolio_vs_market)
    ...
    if not episode % AGGREGATE_STATS_EVERY or episode == 1:
        ...
        max_market_change = max(market_change_list[-AGGREGATE_STATS_EVERY:])
        max_portfolio_change = max(portfolio_change_list[-AGGREGATE_STATS_EVERY:])
        max_portfolio_vs_market = max(portfolio_vs_market_list[-AGGREGATE_STATS_EVERY:])
        
        min_market_change = min(market_change_list[-AGGREGATE_STATS_EVERY:])
        min_portfolio_change = min(portfolio_change_list[-AGGREGATE_STATS_EVERY:])
        min_portfolio_vs_market = min(portfolio_vs_market_list[-AGGREGATE_STATS_EVERY:])
        
        average_market_change = sum(market_change_list[-AGGREGATE_STATS_EVERY:])/len(market_change_list[-AGGREGATE_STATS_EVERY:])
        average_portfolio_change = sum(portfolio_change_list[-AGGREGATE_STATS_EVERY:])/len(portfolio_change_list[-AGGREGATE_STATS_EVERY:])
        average_portfolio_vs_market = sum(portfolio_vs_market_list[-AGGREGATE_STATS_EVERY:])/len(portfolio_vs_market_list[-AGGREGATE_STATS_EVERY:])
        ...
        agent.tensorboard.update_stats(reward_avg=average_reward,
                                       ...
                                       average_buy_choices=average_buy_choices,
                                       average_hold_choices=average_hold_choices, 
                                       average_sell_choices=average_sell_choices,
                                       min_buy_choices=min_buy_choices,
                                       min_hold_choices=min_hold_choices,
                                       min_sell_choices=min_sell_choices,
                                       max_buy_choices=max_buy_choices,
                                       max_hold_choices=max_hold_choices,
                                       max_sell_choices=max_sell_choices,
                                       max_market_change=max_market_change,
                                       max_portfolio_change=max_portfolio_change,
                                       max_portfolio_vs_market=max_portfolio_vs_market,
                                       min_market_change=min_market_change,
                                       min_portfolio_change=min_portfolio_change,
                                       min_portfolio_vs_market=min_portfolio_vs_market,
                                       average_market_change=average_market_change,
                                       average_portfolio_change=average_portfolio_change,
                                       average_portfolio_vs_market=average_portfolio_vs_market)
...
#             agent.model.save(f'/artifacts/models{today}/{MODEL_NAME}__{max_reward:_>7.2f}max_{average_reward:_>7.2f}avg_{min_reward:_>7.2f}min__{int(time.time())}.model')
    ...
    if episode%500 == 0:
        dir = f'/storage/models/tf_v7/latest_{today}/'
